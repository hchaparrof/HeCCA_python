...
def iniciar_algoritmo() -> estado_algoritmo.EstadoAlgoritmo:
  with open("setup.json", "r") as archivo_json:
    ...
    base = pd.read_csv(datos['archivo_base'])
    apoyo = None
    areas = None
    ...
    if datos['organismo'] == 'anla':
      return estado_algoritmo.EstadoAlgoritmo(df_limpio)
    else:
      if datos['existencia_umbrales']:
        return estado_algoritmo.EstadoIdeam(df_limpio, datos['umbrales'])
      return estado_algoritmo.EstadoIdeam(df_limpio)
