...
def prim_ult(df1: pd.DataFrame) -> tuple[datetime.date, datetime.date, int]:
  """
...
def process_df(df_base: pd.DataFrame, df_apoyo: Optional[pd.DataFrame] = None, areas: Optional[tuple] = None, anios_utiles: Optional[List[int]] = None) -> Optional[pd.DataFrame]:
  """
    ...
    """
  df_base, df_apoyo = organize_df(df_base, df_apoyo, anios_utiles)
...
def organize_df(df_base: pd.DataFrame, df_apoyo: Optional[pd.DataFrame] = None, anios_utiles: Optional[List[int]] = None) ->tuple[pd.DataFrame, Optional[pd.DataFrame]]:
  """
  ...
  base = base.reindex(days)
  base, anios = anios_vacios(base)
  anio_inicial, anio_final = base.index[0].year, base.index[-1].year
  if anios_utiles is not None:
    anios_utiles: set[int] = set(anios_utiles)
    base = quitar_anio(base, anios_utiles)
  ...
  if df_apoyo is not None:
    ...
    apoyo = quitar_anio(apoyo, anios)
    if anios_utiles is not None:
      apoyo = quitar_anio(apoyo, anios_utiles)
    return base, apoyo
  else:
    return base, None
...
def quitar_anio(df: Optional[pd.DataFrame], anios: set) -> Optional[pd.DataFrame]:
  if df is None:
    return None
...
def fill_data_na(data: pd.DataFrame) -> pd.DataFrame:
  ...
  def fill_caudal_day(row: pd.Series) -> float:
    """
